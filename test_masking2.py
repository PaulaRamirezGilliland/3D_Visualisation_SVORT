...
def set_opacity(slice_actors1, slice_actors2, slice_actors3, slice_actors4, slice_actors5,# slice_actors6,
                clicked_slice_idx):
    """
    ...
    :param slice_actors5: (Pyvista actor) STIC (full opacity)
    :param slice_actors6: (Pyvista actor) STIC (full opacity)
...
def click_callback(widget, event_name):
    ...
    if closest_slice_idx is not None:
        set_opacity(slice_actors1, slice_actors2, slice_actors3, slice_actors4, slice_actors5, #slice_actors6,
                    closest_slice_idx)
        update_crosshairs([0, 0, closest_slice_idx * volume.GetSpacing()[0]])


def add_slices_to_plotter(plotter, volume_np, useful_slices,
                          transforms, row_index, subplot_index, cmap,
                          opacity, overlay=False):
    """
    ...
    for ind_tr, idx in enumerate(transforms[0]):
        final_transform = compose_transform(transforms[0][ind_tr], transforms[1][ind_tr])
        z_translation = final_transform[2, 3]
        slice_idx = int(round(z_translation / volume.GetSpacing()[2]))

        if slice_idx in useful_slices:
            ...
            if np.any(slice_data):
                ...
                if transforms is not None:
                    ...
                    transform.SetMatrix(np.append(final_transform, [[0, 0, 0, 1]], axis=0).flatten())
...
if __name__ == '__main__':
    ...
    # STIC (predicted slices)
    #plotter.subplot(1, 2)
    #plotter.add_text("STIC (pred) - j=up, k=down", font_size=10)
    #slice_actors6 = add_slices_to_plotter(plotter, volume_np_full, slices_3d, pred_inv_transforms, transforms_gt, 1, 2,
    #                                      "gray", 0.1)
